...
#     return HttpResponse('This portion is currently off for the general user.Go back')


# Multi Line comments. Here is the Adding book segment which will be off for a normal visitor. However We will have a intregated method 
# like user can add books if he is premium user.
def add_book(request):
    if request.method == 'POST':
...
# This result has generated by the genre of the books
def showresultbook(request):
        genre = request.GET.get('genre')
        if genre=='nonfiction':
            data = books.objects.filter(genre__iexact=genre)
        elif genre == 'fiction':
            data = books.objects.filter(genre__iexact=genre)
        elif genre == 'mystery':
            data = books.objects.filter(genre__iexact=genre)
